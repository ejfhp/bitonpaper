...
class PrintSheet extends StatelessWidget {
  ...
  Future<void> preparePrintPreview(int ppp) async {
    ...
    temporarySetBodyStyleNotPositionFixed(true);
    ...
    for (int p = 0; p < numPages; p++) {
      ...
      for (int pp = 0; pp < ppp && wi < wl; pp++) {
        ...
        html.DivElement pDiv = await makePaperDiv(paper: papers[wi++], width: width, height: height);
  ...
  Future<html.DivElement> makePaperDiv({Paper paper, int height, int width}) async {
  ...
  showPrintPreview() {
    html.window.print();
    temporarySetBodyStyleNotPositionFixed(false);
    html.querySelector("#printpreview").children.clear();
  }

  void temporarySetBodyStyleNotPositionFixed(bool removePosition) {
